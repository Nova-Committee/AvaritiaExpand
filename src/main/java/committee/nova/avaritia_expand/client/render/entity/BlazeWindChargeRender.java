...
@OnlyIn(Dist.CLIENT)
public class BlazeWindChargeRender extends EntityRenderer<BlazeWindCharge> {

    private static final float MIN_CAMERA_DISTANCE_SQUARED = Mth.square(3.5F);
    private static final ResourceLocation TEXTURE_LOCATION = AvaritiaExpand.rl("textures/entity/wind_charge/blaze_wind_charge.png");
    private final WindChargeModel model;
    
    public BlazeWindChargeRender(EntityRendererProvider.Context context) {
        super(context);
        this.model = new WindChargeModel(context.bakeLayer(ModelLayers.WIND_CHARGE));
    }
    public void render(BlazeWindCharge entity, float entityYaw, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight) {
    ...
    public ResourceLocation getTextureLocation(BlazeWindCharge entity) {
