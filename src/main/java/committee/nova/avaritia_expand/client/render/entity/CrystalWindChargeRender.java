...
@OnlyIn(Dist.CLIENT)
public class CrystalWindChargeRender extends EntityRenderer<CrystalWindCharge> {

    private static final float MIN_CAMERA_DISTANCE_SQUARED = Mth.square(3.5F);
    private static final ResourceLocation TEXTURE_LOCATION = AvaritiaExpand.rl("textures/entity/wind_charge/crystal_wind_charge.png");
    private final WindChargeModel model;
    
    public CrystalWindChargeRender(EntityRendererProvider.Context context) {
        super(context);
        this.model = new WindChargeModel(context.bakeLayer(ModelLayers.WIND_CHARGE));
    }
    public void render(CrystalWindCharge entity, float entityYaw, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight) {
    ...
    public ResourceLocation getTextureLocation(CrystalWindCharge entity) {
