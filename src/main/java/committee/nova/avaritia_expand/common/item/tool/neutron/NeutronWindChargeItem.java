...
public class NeutronWindChargeItem extends Item implements ProjectileItem {
    private static final int COOLDOWN = 10;

    public NeutronWindChargeItem(Properties properties) {
        super(properties);
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (!level.isClientSide()) {
            NeutronWindCharge NeutronWindCharge = new NeutronWindCharge(player, level, player.position().x(), player.getEyePosition().y(), player.position().z());
            NeutronWindCharge.shootFromRotation(player, player.getXRot(), player.getYRot(), 0.0F, 1.5F, 1.0F);
            level.addFreshEntity(NeutronWindCharge);
        }
    ...
    public Projectile asProjectile(Level level, Position pos, ItemStack stack, Direction direction) {
        ...
        NeutronWindCharge NeutronWindCharge = new NeutronWindCharge(level, pos.x(), pos.y(), pos.z(), vec3);
        NeutronWindCharge.setDeltaMovement(vec3);
        return NeutronWindCharge;
    }
